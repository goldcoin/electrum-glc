    ...
    def validate_params(
        ...
        if not peer_features.supports(LnFeatures.OPTION_SUPPORT_LARGE_CHANNEL_OPT):
            ...
            if funding_sat > LN_MAX_FUNDING_SAT_LEGACY:
                raise Exception(
                    f"funding_sat too high: {funding_sat} sat > {LN_MAX_FUNDING_SAT_LEGACY} (legacy limit)"
                )
        if funding_sat > config.LIGHTNING_MAX_FUNDING_SAT:
            raise Exception(
                f"funding_sat too high: {funding_sat} sat > {config.LIGHTNING_MAX_FUNDING_SAT} (config setting)"
            )
        # MUST set push_msat to equal or less than 1000 * funding_satoshis
        if not (0 <= self.initial_msat <= 1000 * funding_sat):
            raise Exception(
                f"{conf_name}. insane initial_msat={self.initial_msat}. (funding_sat={funding_sat})"
            )
        if self.reserve_sat < self.dust_limit_sat:
            raise Exception(
                f"{conf_name}. MUST set channel_reserve_satoshis greater than or equal to dust_limit_satoshis"
            )
        ...
        if self.reserve_sat > funding_sat // 100:
            raise Exception(
                f"{conf_name}. reserve too high: {self.reserve_sat}, funding_sat: {funding_sat}"
            )
        if self.htlc_minimum_msat > 1_000:
            raise Exception(
                f"{conf_name}. htlc_minimum_msat too high: {self.htlc_minimum_msat} msat"
            )
        HTLC_MINIMUM_MSAT_MIN = (
            0  # should be at least 1 really, but apparently some nodes are sending zero...
        )
        if self.htlc_minimum_msat < HTLC_MINIMUM_MSAT_MIN:
            raise Exception(
                f"{conf_name}. htlc_minimum_msat too low: {self.htlc_minimum_msat} msat < {HTLC_MINIMUM_MSAT_MIN}"
            )
        ...
        if self.to_self_delay > MAXIMUM_REMOTE_TO_SELF_DELAY_ACCEPTED:
            raise Exception(
                f"{conf_name}. to_self_delay too high: {self.to_self_delay} > {MAXIMUM_REMOTE_TO_SELF_DELAY_ACCEPTED}"
            )
        if self.max_htlc_value_in_flight_msat < min(1000 * funding_sat, 100_000_000):
            raise Exception(
                f"{conf_name}. max_htlc_value_in_flight_msat is too small: {self.max_htlc_value_in_flight_msat}"
            )
    ...
    def cross_validate_params(
        ...
        *,
        ...
        if remote_config.reserve_sat < local_config.dust_limit_sat:
            raise Exception(
                "violated constraint: remote_config.reserve_sat < local_config.dust_limit_sat"
            )
        # if channel_reserve_satoshis from the open_channel message is less than dust_limit_satoshis:
        #     MUST reject the channel.
        if local_config.reserve_sat < remote_config.dust_limit_sat:
            raise Exception(
                "violated constraint: local_config.reserve_sat < remote_config.dust_limit_sat"
            )
        # The receiving node MUST fail the channel if:
        #     the funder's amount for the initial commitment transaction is not
        #     sufficient for full fee payment.
        if (
            funder_config.initial_msat
            < calc_fees_for_commitment_tx(
                num_htlcs=0, feerate=initial_feerate_per_kw, is_local_initiator=is_local_initiator
            )[funder]
        ):
            raise Exception(
        ...
        if max(local_config.initial_msat, remote_config.initial_msat) <= 1000 * max(
            local_config.reserve_sat, remote_config.reserve_sat
        ):
            raise Exception(
...
class ConnStringFormatError(LightningError):
    pass


class RemoteMisbehaving(LightningError):
    pass


class NotFoundChanAnnouncementForUpdate(Exception):
    pass
...
class PaymentFailure(UserFacingException):
    pass
...
def shachain_derive(element, to_index):
    ...
    return ShachainElement(
        get_per_commitment_secret_from_seed(element.secret, to_index, zeros), to_index
    )
...
def derive_pubkey(basepoint: bytes, per_commitment_point: bytes) -> bytes:
    p = ecc.ECPubkey(basepoint) + ecc.GENERATOR * ecc.string_to_number(
...
def derive_blinded_pubkey(basepoint: bytes, per_commitment_point: bytes) -> bytes:
    ...
    k2 = ecc.ECPubkey(per_commitment_point) * ecc.string_to_number(
        sha256(per_commitment_point + basepoint)
    )
...
def derive_blinded_privkey(basepoint_secret: bytes, per_commitment_secret: bytes) -> bytes:
    ...
    per_commitment_point = ecc.ECPrivkey(per_commitment_secret).get_public_key_bytes(
        compressed=True
    )
    k1 = ecc.string_to_number(basepoint_secret) * ecc.string_to_number(
        sha256(basepoint + per_commitment_point)
    )
    k2 = ecc.string_to_number(per_commitment_secret) * ecc.string_to_number(
        sha256(per_commitment_point + basepoint)
    )
...
def make_htlc_tx_witness(
    remotehtlcsig: bytes, localhtlcsig: bytes, payment_preimage: bytes, witness_script: bytes
) -> bytes:
    ...
    return bfh(
        construct_witness([0, remotehtlcsig, localhtlcsig, payment_preimage, witness_script])
    )
...
def make_htlc_tx_inputs(
    ...
    txin = PartialTxInput(
        prevout=TxOutpoint(txid=bfh(htlc_output_txid), out_idx=htlc_output_index), nsequence=0
    )
...
def map_htlcs_to_ctx_output_idxs(
    *, chan: "Channel", ctx: Transaction, pcp: bytes, subject: "HTLCOwner", ctn: int
    ...
    inverse_map = {
        ctx_output_idx: (direction, htlc)
        for ((direction, htlc), ctx_output_idx) in htlc_to_ctx_output_idx_map.items()
    }

    return {
        inverse_map[ctx_output_idx]: (ctx_output_idx, htlc_relative_idx)
...
def make_htlc_tx_with_open_channel(
    *,
    ...
    htlc_tx_inputs = make_htlc_tx_inputs(
        commit.txid(), ctx_output_idx, amount_msat=amount_msat, witness_script=preimage_script.hex()
    )
    if is_htlc_success:
        cltv_abs = 0
...
def make_commitment_outputs(
    *,
    ...
    for script, htlc in htlcs:
        ...
        htlc_outputs.append(
            PartialTxOutput(
                scriptpubkey=bfh(address_to_script(addr)), value=htlc.amount_msat // 1000
            )
        )

    # trim outputs
    c_outputs_filtered = list(
        filter(lambda x: x.value >= dust_limit_sat, non_htlc_outputs + htlc_outputs)
    )
...
def make_commitment(
    *,
    ...
    local_address = make_commitment_output_to_local_address(
...
def make_commitment_output_to_local_address(
    ...
    local_script = make_commitment_output_to_local_witness_script(
        revocation_pubkey, to_self_delay, delayed_pubkey
    )
...
def funding_output_script(local_config, remote_config) -> str:
    return funding_output_script_from_keys(
        local_config.multisig_key.pubkey, remote_config.multisig_key.pubkey
    )
...
class LnFeatures(IntFlag):
    ...
    _ln_feature_contexts[OPTION_TRAMPOLINE_ROUTING_REQ_ECLAIR] = (
        LNFC.INIT | LNFC.NODE_ANN | LNFC.INVOICE
    )
    _ln_feature_contexts[OPTION_TRAMPOLINE_ROUTING_OPT_ECLAIR] = (
        LNFC.INIT | LNFC.NODE_ANN | LNFC.INVOICE
    )
    ...
    _ln_feature_contexts[OPTION_TRAMPOLINE_ROUTING_REQ_ELECTRUM] = (
        LNFC.INIT | LNFC.NODE_ANN | LNFC.INVOICE
    )
    _ln_feature_contexts[OPTION_TRAMPOLINE_ROUTING_OPT_ELECTRUM] = (
        LNFC.INIT | LNFC.NODE_ANN | LNFC.INVOICE
    )
...
class UnknownEvenFeatureBits(IncompatibleOrInsaneFeatures):
    pass


class IncompatibleLightningFeatures(IncompatibleOrInsaneFeatures):
    pass
...
def ln_compare_features(our_features: "LnFeatures", their_features: int) -> "LnFeatures":
    ...
    for flag in our_flags:
        if flag not in their_flags and get_ln_flag_pair_of_bit(flag) not in their_flags:
            ...
            if flag % 2 == 0:  # even flags are compulsory
                raise IncompatibleLightningFeatures(
                    f"remote does not support {LnFeatures(1 << flag)!r}"
                )
    ...
    for flag in their_flags:
        if flag not in our_flags and get_ln_flag_pair_of_bit(flag) not in our_flags:
            ...
            if flag % 2 == 0:  # even flags are compulsory
                raise IncompatibleLightningFeatures(
                    f"remote wanted feature we don't have: {LnFeatures(1 << flag)!r}"
                )
    return our_features
...
if hasattr(sys, "get_int_max_str_digits"):
    ...
    ), f"sys.get_int_max_str_digits() too low: {sys.get_int_max_str_digits()}"
...
def validate_features(features: int) -> LnFeatures:
    ...
    if features.bit_length() > 10_000:
        ...
        raise IncompatibleOrInsaneFeatures(
            f"features bitvector too large: {features.bit_length()=} > 10_000"
        )
...
class LNPeerAddr:
    ...
    def __init__(self, host: str, port: int, pubkey: bytes):
        ...
        try:
            ...
            raise ValueError(
                f"cannot construct LNPeerAddr: invalid host or port (host={host}, port={port})"
            ) from e
...
def split_host_port(host_port: str) -> tuple[str, str]:  # port returned as string
    ...
    if not m:
        raise ConnStringFormatError(
            _("Connection strings must be in <node_pubkey>@<host>:<port> format")
        )
    host = m.group("host")
    if m.group("port"):
        port = m.group("port")[1:]
...
class UpdateAddHtlc:
    ...
    payment_hash = attr.ib(
        type=bytes, kw_only=True, converter=hex_to_bytes, repr=lambda val: val.hex()
    )
