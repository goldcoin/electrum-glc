...
class HTLCManager:
        ...
        if initial_feerate is not None:
            ...
            for sub in (LOCAL, REMOTE):
                if not log[sub]["fee_updates"]:
                    log[sub]["fee_updates"][0] = FeeUpdate(
                        rate=initial_feerate, ctn_local=0, ctn_remote=0
                    )
        self.log = log
    ...
    def send_htlc(self, htlc: UpdateAddHtlc) -> UpdateAddHtlc:
        ...
        if htlc_id != self.get_next_htlc_id(LOCAL):
            raise Exception(
                f"unexpected local htlc_id. next should be "
    ...
    def recv_htlc(self, htlc: UpdateAddHtlc) -> None:
        ...
        if htlc_id != self.get_next_htlc_id(REMOTE):
            raise Exception(
                f"unexpected remote htlc_id. next should be "
    ...
    def _new_feeupdate(self, fee_update: FeeUpdate, subject: HTLCOwner) -> None:
        ...
        if (
            last_fee_update.ctn_local is None or last_fee_update.ctn_local > self.ctn_latest(LOCAL)
        ) and (
            last_fee_update.ctn_remote is None
            or last_fee_update.ctn_remote > self.ctn_latest(REMOTE)
        ):
            d[n - 1] = fee_update
        else:
            d[n] = fee_update
    ...
    def send_ctx(self) -> None:
        assert self.ctn_latest(REMOTE) == self.ctn_oldest_unrevoked(REMOTE), (
            self.ctn_latest(REMOTE),
            self.ctn_oldest_unrevoked(REMOTE),
        )
    ...
    def recv_ctx(self) -> None:
        assert self.ctn_latest(LOCAL) == self.ctn_oldest_unrevoked(LOCAL), (
            self.ctn_latest(LOCAL),
            self.ctn_oldest_unrevoked(LOCAL),
        )
    ...
    def _init_maybe_active_htlc_ids(self):
        ...
        self._maybe_active_htlc_ids = {
            LOCAL: set(),
            REMOTE: set(),
        }  # type: Dict[HTLCOwner, Set[int]]
        # add all htlcs
    ...
    @with_lock
    def store_local_update_raw_msg(
        self, raw_update_msg: bytes, *, is_commitment_signed: bool
    ) -> None:
    ...
    @with_lock
    def is_htlc_active_at_ctn(
        self, *, ctx_owner: HTLCOwner, ctn: int, htlc_proposer: HTLCOwner, htlc_id: int
    ) -> bool:
        htlc_id = int(htlc_id)
        ...
        if ctns[ctx_owner] is not None and ctns[ctx_owner] <= ctn:
            not_settled = (
                htlc_id not in settles
                or settles[htlc_id][ctx_owner] is None
                or settles[htlc_id][ctx_owner] > ctn
            )
            not_failed = (
                htlc_id not in fails
                or fails[htlc_id][ctx_owner] is None
                or fails[htlc_id][ctx_owner] > ctn
            )
    ...
    def htlcs_by_direction(
        ...
        for htlc_id in considered_htlc_ids:
            htlc_id = int(htlc_id)
            if self.is_htlc_active_at_ctn(
                ctx_owner=subject, ctn=ctn, htlc_proposer=party, htlc_id=htlc_id
            ):
    ...
    def all_settled_htlcs_ever(
        ...
        received = [
            (RECEIVED, x) for x in self.all_settled_htlcs_ever_by_direction(subject, RECEIVED, ctn)
        ]
    ...
    def received_in_ctn(self, local_ctn: int) -> Sequence[UpdateAddHtlc]:
        ...
        """
        return self._get_htlcs_that_got_removed_exactly_at_ctn(
            local_ctn, ctx_owner=LOCAL, htlc_proposer=REMOTE, log_action="settles"
        )
    ...
    def sent_in_ctn(self, remote_ctn: int) -> Sequence[UpdateAddHtlc]:
        ...
        """
        return self._get_htlcs_that_got_removed_exactly_at_ctn(
            remote_ctn, ctx_owner=REMOTE, htlc_proposer=LOCAL, log_action="settles"
        )
    ...
    def failed_in_ctn(self, remote_ctn: int) -> Sequence[UpdateAddHtlc]:
        ...
        """
        return self._get_htlcs_that_got_removed_exactly_at_ctn(
            remote_ctn, ctx_owner=REMOTE, htlc_proposer=LOCAL, log_action="fails"
        )
    ...
    def get_feerate(self, subject: HTLCOwner, ctn: int) -> int:
        ...
        assert not (
            len(self.log[LOCAL]["fee_updates"]) > 1 and len(self.log[REMOTE]["fee_updates"]) > 1
        )
