...
class MockInterface(Interface):
    def __init__(self, config):
        ...
        super().__init__(
            network=network, server=ServerAddr.from_str("mock-server:50000:t"), proxy=None
        )
...
class TestNetwork(ElectrumTestCase):
    ...
    async def test_fork_noconflict(self):
        ...
        self.interface.q.put_nowait(
            {
                "block_height": 2,
                "mock": {"backward": 1, "check": lambda x: True, "connect": lambda x: False},
            }
        )
        self.interface.q.put_nowait(
            {
                "block_height": 4,
                "mock": {"binary": 1, "check": lambda x: True, "connect": lambda x: True},
            }
        )
        self.interface.q.put_nowait(
            {
    ...
    async def test_can_connect_during_backward(self):
        ...
        self.interface.q.put_nowait(
            {
                "block_height": 3,
                "mock": {"catchup": 1, "check": lambda x: False, "connect": lambda x: True},
            }
        )
        self.interface.q.put_nowait(
            {
    ...
    def mock_fork(self, bad_header):
        ...
        b = blockchain.Blockchain(
            ...
            forkpoint=forkpoint,
            parent=None,
            forkpoint_hash=sha256(str(forkpoint)).hex(),
            prev_hash=sha256(str(forkpoint - 1)).hex(),
        )
        return b
    ...
    async def test_chain_false_during_binary(self):
        ...
        self.interface.q.put_nowait(
            {
                "block_height": 7,
                "mock": {"backward": 1, "check": lambda x: False, "connect": mock_connect},
            }
        )
        self.interface.q.put_nowait(
            {
                "block_height": 2,
                "mock": {"backward": 1, "check": lambda x: True, "connect": mock_connect},
            }
        )
        self.interface.q.put_nowait(
            {
