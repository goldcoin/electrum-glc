...
class TestWalletPaymentRequests(ElectrumTestCase):
    ...
    async def test_wallet_with_ln_creates_payreq_and_gets_paid_on_ln(self):
        ...
        pr_key = wallet1.create_request(
            amount_sat=10000, message="msg", address=addr, exp_delay=86400
        )
    ...
    async def test_wallet_with_ln_creates_payreq_and_gets_paid_onchain(self):
        ...
        tx_info = TxMinedInfo(
            height=1001, timestamp=pr.get_time() + 100, txpos=1, header_hash="01" * 32
        )
    ...
    async def test_wallet_gets_paid_onchain_in_the_past(self):
        ...
        tx_info = TxMinedInfo(
            height=990, timestamp=pr.get_time() + 100, txpos=1, header_hash="01" * 32
        )
        wallet1.adb.add_verified_tx(tx.txid(), tx_info)
        self.assertEqual(PR_UNPAID, wallet1.get_invoice_status(pr))
    ...
    async def test_wallet_reuse_unused_fallback_onchain_addr_when_getting_paid_with_lightning(self):
        ...
        pr1_key = wallet1.create_request(
            amount_sat=10000, message="msg", address=addr1, exp_delay=86400
        )
        ...
        pr2_key = wallet1.create_request(
            amount_sat=10000, message="msg", address=addr2, exp_delay=86400
        )
        ...
        pr3_key = wallet1.create_request(
            amount_sat=10000, message="msg", address=addr3, exp_delay=86400
        )
        ...
        pr4_key = wallet1.create_request(
            amount_sat=10000, message="msg", address=addr4, exp_delay=86400
        )
    ...
    async def test_arg_validation(self):
        ...
        outputs = [
            PartialTxOutput.from_address_and_value(
                "tb1qmjzmg8nd4z56ar4fpngzsr6euktrhnjg9td385", amount_sat
            )
        ]
        invoice = Invoice(
