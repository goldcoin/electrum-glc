    ...
    def test_mnemonic_to_seed(self):
        for test_name, test in SEED_TEST_CASES.items():
            ...
            if test.passphrase_hex is not None:
                self.assertEqual(
                    test.passphrase_hex, test.passphrase.encode("utf8").hex(), msg=test_name
                )
...
class Test_seeds(ElectrumTestCase):
    ...
    def test_is_matching_seed(self):
        ...
        self.assertTrue(
            is_matching_seed(seed="when blade focus", seed_again=" when  blade       focus  ")
        )
        self.assertTrue(
            is_matching_seed(seed=" when  blade  focus  ", seed_again=" when  blade       focus  ")
        )
