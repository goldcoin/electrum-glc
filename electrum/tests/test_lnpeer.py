...
class MockLNWallet(Logger, EventListener, NetworkRetryManager[LNPeerAddr]):
    ...
    async def create_routes_from_invoice(
        self, amount_msat: int, decoded_invoice: LnAddr, *, full_path=None
    ):
        ...
        return [
            r
            async for r in self.create_routes_for_payment(
    ...
    wait_for_received_pending_htlcs_to_get_removed = (
        LNWallet.wait_for_received_pending_htlcs_to_get_removed
    )
    ...
    def send_bytes(self, data):
        ...
        if decoded[0] == "init":
            self.queue.put_nowait(
                encode_msg("init", lflen=1, gflen=1, localfeatures=b"\x00", globalfeatures=b"\x00")
            )
...
class SuccessfulTest(Exception):
    pass
            ...
            async def cb(payment_hash):
                if not test_failure:
                    ...
                    raise OnionRoutingFailure(
                        code=OnionFailureCode.INCORRECT_OR_UNKNOWN_PAYMENT_DETAILS, data=b""
                    )
...
class TestPeerDirect(TestPeer):
    ...
    def prepare_peers(
        ...
        *,
        ...
        w1 = MockLNWallet(
            local_keypair=k1, chans=[alice_channel], tx_queue=q1, name=bob_channel.name
        )
        w2 = MockLNWallet(
            local_keypair=k2, chans=[bob_channel], tx_queue=q2, name=alice_channel.name
        )
    ...
    async def test_reestablish(self):
        ...
        async def reestablish():
            await asyncio.gather(
                p1.reestablish_channel(alice_channel), p2.reestablish_channel(bob_channel)
            )
    ...
    async def test_reestablish_with_old_state(self):
        ...
        alice_channel_0, bob_channel_0 = create_test_channels(
        ...
        gath = asyncio.gather(
            pay(), p1._message_loop(), p2._message_loop(), p1.htlc_switch(), p2.htlc_switch()
        )
        with self.assertRaises(asyncio.CancelledError):
            await gath
        ...
        async def reestablish():
            await asyncio.gather(
                p1.reestablish_channel(alice_channel_0), p2.reestablish_channel(bob_channel)
            )
    ...
    def _send_fake_htlc(peer: Peer, chan: Channel) -> UpdateAddHtlc:
        htlc = UpdateAddHtlc(
            amount_msat=10000, payment_hash=os.urandom(32), cltv_abs=999, timestamp=1
        )
    ...
    async def test_reestablish_replay_messages_rev_then_sig(self):
        ...
        async def f():
            ...
            async with OldTaskGroup() as group:
                ...
                self.assertTrue(
                    any(
                        ("alice->bob" in msg and "replaying a revoke_and_ack first" in msg)
                ...
                self.assertTrue(
                    any(
                        (
                            "alice->bob" in msg
                            and "replayed 2 unacked messages. ['update_add_htlc', 'commitment_signed']"
                            in msg
                        )
                        for msg in logs.output
                    )
    ...
    async def test_reestablish_replay_messages_sig_then_rev(self):
        ...
        async def f():
            ...
            async with OldTaskGroup() as group:
                ...
                self.assertTrue(
                    any(
                        ("alice->bob" in msg and "replaying a revoke_and_ack last" in msg)
                        for msg in logs.output
                    )
                )
                self.assertTrue(
        ...
        for test_trampoline in [False, True]:
            with self.assertRaises(PaymentFailure):
                await self._test_simple_payment(
                    test_trampoline=test_trampoline, test_bundle=True, test_bundle_timeout=True
                )
    ...
    async def test_simple_payment_success_with_hold_invoice(self):
        for test_trampoline in [False, True]:
            with self.assertRaises(PaymentDone):
                await self._test_simple_payment(
                    test_trampoline=test_trampoline, test_hold_invoice=True
                )
    ...
    async def test_simple_payment_failure_with_hold_invoice(self):
        for test_trampoline in [False, True]:
            with self.assertRaises(PaymentFailure):
                await self._test_simple_payment(
                    test_trampoline=test_trampoline, test_hold_invoice=True, test_failure=True
                )
    ...
    async def test_payment_race(self):
        ...
        async def pay():
            ...
            route1 = (
                await w1.create_routes_from_invoice(
                    lnaddr2.get_amount_msat(), decoded_invoice=lnaddr2
                )
            )[0][0].route
            ...
            route2 = (
                await w2.create_routes_from_invoice(
                    lnaddr1.get_amount_msat(), decoded_invoice=lnaddr1
                )
            )[0][0].route
    ...
    async def test_payments_stresstest(self):
        ...
        num_payments = 50
        payment_value_msat = (
            10_000_000  # make it large enough so that there are actually HTLCs on the ctx
        )
        ...
        with self.assertRaises(asyncio.CancelledError):
            await gath
        self.assertEqual(
            alice_init_balance_msat - num_payments * payment_value_msat,
            alice_channel.balance(HTLCOwner.LOCAL),
        )
        self.assertEqual(
            alice_init_balance_msat - num_payments * payment_value_msat,
            bob_channel.balance(HTLCOwner.REMOTE),
        )
        self.assertEqual(
            bob_init_balance_msat + num_payments * payment_value_msat,
            bob_channel.balance(HTLCOwner.LOCAL),
        )
        self.assertEqual(
            bob_init_balance_msat + num_payments * payment_value_msat,
            alice_channel.balance(HTLCOwner.REMOTE),
        )
    ...
    async def test_payment_recv_mpp_confusion1(self):
        ...
        async def pay():
            ...
            route = (
                await w1.create_routes_from_invoice(amount_msat=1000, decoded_invoice=lnaddr1)
            )[0][0].route
            p1.pay(
                route=route,
                chan=alice_channel,
    ...
    async def _test_shutdown(self, alice_fee, bob_fee, alice_fee_range=None, bob_fee_range=None):
        ...
        async def pay():
            ...
            route = (
                await w1.create_routes_from_invoice(
                    lnaddr.get_amount_msat(), decoded_invoice=lnaddr
                )
            )[0][0].route
            p1.pay(
                route=route,
    ...
    async def test_warning(self):
        ...
        gath = asyncio.gather(
            action(), p1._message_loop(), p2._message_loop(), p1.htlc_switch(), p2.htlc_switch()
        )
    ...
    async def test_error(self):
        ...
        async def action():
            ...
            await util.wait_for2(p2.initialized, 1)
            await p1.send_error(
                alice_channel.channel_id, "some error happened!", force_close_channel=True
            )
    ...
    async def test_sending_weird_messages__unknown_even_type(self):
        ...
        with self.assertRaises(GracefulDisconnect):
            await f()
        self.assertTrue(
            isinstance(failing_task.exception().__cause__, lnmsg.UnknownMandatoryMsgType)
        )
...
class TestPeerForwarding(TestPeer):
    ...
    def prepare_chans_and_peers_in_graph(self, graph_definition) -> Graph:
        ...
        for a, definition in graph_definition.items():
            for b, channel_def in definition.get("channels", {}).items():
                ...
                transport_ab, transport_ba = transport_pair(
                    keys[a], keys[b], channel_ab.name, channel_ba.name
                )
                transports[(a, b)], transports[(b, a)] = transport_ab, transport_ba
                # set fees
                channel_ab.forwarding_fee_proportional_millionths = channel_def[
                    "local_fee_rate_millionths"
                ]
                channel_ab.forwarding_fee_base_msat = channel_def["local_base_fee_msat"]
                channel_ba.forwarding_fee_proportional_millionths = channel_def[
                    "remote_fee_rate_millionths"
                ]
        ...
        for a, definition in graph_definition.items():
            ...
            workers[a] = MockLNWallet(
                local_keypair=keys[a], chans=channels_of_node, tx_queue=txs_queues[a], name=a
            )
    ...
    async def test_payment_multihop(self):
        ...
        async def pay(lnaddr, pay_req):
            self.assertEqual(
                PR_UNPAID, graph.workers["dave"].get_payment_status(lnaddr.paymenthash)
            )
        ...
        async def f():
            async with OldTaskGroup() as group:
                ...
                lnaddr, pay_req = self.prepare_invoice(
                    graph.workers["dave"], include_routing_hints=True
                )
                await group.spawn(pay(lnaddr, pay_req))
        ...
        async def pay(lnaddr, pay_req):
            ...
            self.assertEqual(
                [
                    ...
                    graph.channels[("carol", "dave")].short_channel_id,
                ],
                [edge.short_channel_id for edge in log[0].route],
            )
            self.assertEqual(
                [
                    graph.channels[("alice", "bob")].short_channel_id,
                    graph.channels[("bob", "dave")].short_channel_id,
                ],
    ...
    async def test_refuse_to_forward_htlc_that_corresponds_to_payreq_we_created(self):
        ...
        async def pay():
            ...
            with self.subTest(msg="try to make Bob forward in trampoline mode"):
                ...
                electrum.trampoline._TRAMPOLINE_NODES_UNITTESTS = {
                    graph.workers["bob"].name: LNPeerAddr(
                        host="127.0.0.1", port=9735, pubkey=graph.workers["bob"].node_keypair.pubkey
                    ),
                }
        ...
        async def f():
            async with OldTaskGroup() as group:
                ...
                for peer in peers:
                    await peer.initialized
                lnaddr, pay_req = self.prepare_invoice(
                    graph.workers["dave"], amount_msat=amount_to_pay, include_routing_hints=True
                )
                await group.spawn(pay(lnaddr, pay_req))
    ...
    async def _run_mpp(self, graph, kwargs):
        ...
        async def pay(
            ...
            lnaddr, pay_req = self.prepare_invoice(
                dave_w, include_routing_hints=True, amount_msat=amount_to_pay
            )
    ...
    async def test_payment_multipart_trampoline_e2e(self):
        ...
        electrum.trampoline._TRAMPOLINE_NODES_UNITTESTS = {
            ...
            graph.workers["carol"].name: LNPeerAddr(
                host="127.0.0.1", port=9735, pubkey=graph.workers["carol"].node_keypair.pubkey
            ),
        }
    ...
    async def test_payment_multipart_trampoline_legacy(self):
        ...
        with self.assertRaises(NoPathFound):
            await self._run_mpp(
                graph,
                {
                    "alice_uses_trampoline": True,
                    "attempts": 30,
                    "disable_trampoline_receiving": True,
                },
            )
    ...
    async def test_fail_pending_htlcs_on_shutdown(self):
        ...
        async def pay():
            ...
            lnaddr, pay_req = self.prepare_invoice(
                graph.workers["dave"], include_routing_hints=True, amount_msat=amount_to_pay
            )
    ...
    async def _run_trampoline_payment(
        ...
        *,
        ...
        async def pay(lnaddr, pay_req):
            ...
            if result:
                self.assertEqual(
                    PR_PAID, graph.workers["dave"].get_payment_status(lnaddr.paymenthash)
                )
        ...
        async def f():
            ...
            async with OldTaskGroup() as group:
                ...
                for peer in peers:
                    await peer.initialized
                lnaddr, pay_req = self.prepare_invoice(
                    dave_w, include_routing_hints=include_routing_hints
                )
                self.prepare_recipient(dave_w, lnaddr.paymenthash, test_hold_invoice, test_failure)
                await group.spawn(pay(lnaddr, pay_req))
    ...
    async def test_trampoline_mpp_consolidation(self):
        with self.assertRaises(PaymentDone):
            graph = self.create_square_graph(
                direct=False, test_mpp_consolidation=True, is_legacy=True
            )
