...
class TestWalletStorage(WalletTestCase):
    ...
    async def test_storage_imported_add_privkeys_persistence_test(self):
        ...
        wallet.import_private_keys(
            ["p2wpkh:KzuqaaLp9zYjVuj8vQtCwFdiZFreW3NJNBachgVS8S9XMgj5y78b"], password=None
        )
        ...
        self.assertEqual(3, len(wallet.keystore.keypairs))
        self.assertTrue(
            "03bf450797034dc95693096e575e3b3db14e5f074679b349b727f90fc7804ce7ab"
            in wallet.keystore.keypairs
        )
        self.assertTrue(
            "030dac677b9484e23db6f9255eddf433f4f12c02f9b35e0100f2f103ffbccf540f"
            in wallet.keystore.keypairs
        )
        self.assertTrue(
            "02f11d5f222a728fd08226cb5a1e85a74d58fc257bd3764bf1234346f91defed72"
            in wallet.keystore.keypairs
        )
...
class FakeExchange(ExchangeBase):
    def __init__(self, rate):
        ...
        self._quotes_timestamp = float(
            "inf"
        )  # spot price from the far future never becomes stale :P
...
class TestFiat(ElectrumTestCase):
    ...
    def test_save_fiat_and_reset(self):
        self.assertEqual(
            False,
            Abstract_Wallet.set_fiat_value(
                self.wallet, txid, ccy, "1000.01", self.fx, self.value_sat
            ),
        )
        ...
        self.assertEqual(
            True,
            Abstract_Wallet.set_fiat_value(self.wallet, txid, ccy, "", self.fx, self.value_sat),
        )
        ...
        self.assertEqual(
            True,
            Abstract_Wallet.set_fiat_value(
                self.wallet, txid, ccy, "1 000.002", self.fx, self.value_sat
            ),
        )
    ...
    def test_too_high_precision_value_resets_with_no_saved_value(self):
        self.assertEqual(
            True,
            Abstract_Wallet.set_fiat_value(
                self.wallet, txid, ccy, "1 000.001", self.fx, self.value_sat
            ),
        )
    ...
    def test_save_garbage(self):
        self.assertEqual(
            False,
            Abstract_Wallet.set_fiat_value(
                self.wallet, txid, ccy, "garbage", self.fx, self.value_sat
            ),
        )
        self.assertNotIn(ccy, self.fiat_value)
...
class TestCreateRestoreWallet(WalletTestCase):
    ...
    async def test_restore_wallet_from_text_mnemonic(self):
        ...
        self.assertEqual(encrypt_file, wallet.storage.is_encrypted())
        self.assertEqual(
            "bc1q2ccr34wzep58d4239tl3x3734ttle92a8srmuw", wallet.get_receiving_addresses()[0]
        )
    ...
    async def test_restore_wallet_from_text_no_storage(self):
        ...
        self.assertEqual(
            "bc1q3g5tmkmlvxryhh843v4dz026avatc0zzr6h3af", wallet.get_receiving_addresses()[0]
        )
    ...
    async def test_restore_wallet_from_text_addresses(self):
        text = (
            "bc1q2ccr34wzep58d4239tl3x3734ttle92a8srmuw bc1qnp78h78vp92pwdwq5xvh8eprlga5q8gu66960c"
        )
        d = restore_wallet_from_text(text, path=self.wallet_path, config=self.config)
        wallet = d["wallet"]  # type: Imported_Wallet
        self.assertEqual(
