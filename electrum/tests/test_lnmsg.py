...
from electrum.lnmsg import (
    read_bigsize_int,
    write_bigsize_int,
    FieldEncodingNotMinimal,
    UnexpectedEndOfStream,
    LNSerializer,
    UnknownMandatoryTLVRecordType,
    MalformedMsg,
    MsgTrailingGarbage,
    MsgInvalidFieldOrder,
    encode_msg,
    decode_msg,
    UnexpectedFieldSizeForEncoder,
    OnionWireSerializer,
    UnknownMsgType,
)
...
class TestLNMsg(ElectrumTestCase):
    ...
    def test_read_bigsize_int(self):
        ...
        self.assertEqual(
            18446744073709551615, read_bigsize_int(io.BytesIO(bfh("ffffffffffffffffff")))
        )
    ...
    def test_read_tlv_stream_tests1(self):
            ...
            with self.subTest(tlv_stream_name=tlv_stream_name):
                ...
                with self.assertRaises(UnexpectedEndOfStream):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fd01")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(FieldEncodingNotMinimal):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fd000100")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(UnexpectedEndOfStream):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fd0101")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(UnexpectedEndOfStream):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("0ffd")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(UnexpectedEndOfStream):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("0ffd26")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(UnexpectedEndOfStream):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("0ffd2602")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(FieldEncodingNotMinimal):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("0ffd000100")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(UnexpectedEndOfStream):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(
                ...
                with self.assertRaises(UnknownMandatoryTLVRecordType):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("1200")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(UnknownMandatoryTLVRecordType):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fd010200")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(UnknownMandatoryTLVRecordType):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fe0100000200")), tlv_stream_name=tlv_stream_name
                    )
                with self.assertRaises(UnknownMandatoryTLVRecordType):
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("ff010000000000000200")), tlv_stream_name=tlv_stream_name
                    )
        with self.assertRaises(MsgTrailingGarbage):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("0109ffffffffffffffffff")), tlv_stream_name="n1"
            )
        with self.assertRaises(FieldEncodingNotMinimal):
        ...
        with self.assertRaises(MsgTrailingGarbage):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("0209010101010101010101")), tlv_stream_name="n1"
            )
        with self.assertRaises(UnexpectedEndOfStream):
            lnser.read_tlv_stream(
                fd=io.BytesIO(
    ...
    def test_read_tlv_stream_tests2(self):
        ...
        for tlv_stream_name in ("n1", "n2"):
            with self.subTest(tlv_stream_name=tlv_stream_name):
                self.assertEqual(
                    {},
                    lnser.read_tlv_stream(fd=io.BytesIO(bfh("")), tlv_stream_name=tlv_stream_name),
                )
                self.assertEqual(
                    {},
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("2100")), tlv_stream_name=tlv_stream_name
                    ),
                )
                self.assertEqual(
                    {},
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fd020100")), tlv_stream_name=tlv_stream_name
                    ),
                )
                self.assertEqual(
                    {},
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fd00fd00")), tlv_stream_name=tlv_stream_name
                    ),
                )
                self.assertEqual(
                    {},
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fd00ff00")), tlv_stream_name=tlv_stream_name
                    ),
                )
                self.assertEqual(
                    {},
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("fe0200000100")), tlv_stream_name=tlv_stream_name
                    ),
                )
                self.assertEqual(
                    {},
                    lnser.read_tlv_stream(
                        fd=io.BytesIO(bfh("ff020000000000000100")), tlv_stream_name=tlv_stream_name
                    ),
                )
        ...
        self.assertEqual(
            {"tlv1": {"amount_msat": 1099511627776}},
        ...
        self.assertEqual(
            {"tlv1": {"amount_msat": 72057594037927936}},
            lnser.read_tlv_stream(fd=io.BytesIO(bfh("01080100000000000000")), tlv_stream_name="n1"),
        )
        self.assertEqual(
            {"tlv2": {"scid": ShortChannelID.from_components(0, 0, 550)}},
    ...
    def test_read_tlv_stream_tests3(self):
        ...
        with self.assertRaises(MsgInvalidFieldOrder):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("0208000000000000022601012a")), tlv_stream_name="n1"
            )
        with self.assertRaises(MsgInvalidFieldOrder):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("0208000000000000023102080000000000000451")), tlv_stream_name="n1"
            )
        with self.assertRaises(MsgInvalidFieldOrder):
        ...
        with self.assertRaises(MsgInvalidFieldOrder):
            lnser.read_tlv_stream(
                fd=io.BytesIO(bfh("ffffffffffffffffff000000")), tlv_stream_name="n2"
            )
    ...
    def test_encode_decode_msg__missing_mandatory_field_gets_set_to_zeroes(self):
        ...
        self.assertEqual(
            bfh(
                ""
            ),
            encode_msg(
                "channel_update",
    ...
    def test_encode_decode_msg__ints_can_be_passed_as_bytes(self):
        self.assertEqual(
            ...
            encode_msg(
                ...
                htlc_maximum_msat=int.to_bytes(
                    1_000_000_000, length=8, byteorder="big", signed=False
                ),
                fee_base_msat=int.to_bytes(500, length=4, byteorder="big", signed=False),
                fee_proportional_millionths=int.to_bytes(
                    35, length=4, byteorder="big", signed=False
                ),
                chain_hash=constants.net.rev_genesis_bytes(),
    ...
    def test_encode_decode_msg__commitment_signed(self):
        ...
        self.assertEqual(
            bfh(
                ""
            ),
            encode_msg(
                "commitment_signed",
        ...
        self.assertEqual(
            bfh(
                ""
            ),
            encode_msg(
                "commitment_signed",
                channel_id=b"\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01",
        ...
        self.assertEqual(
            bfh(
                "008401010101010101010101010101010101010101010101010101010101010101014e206ecf904d9237b1c5b4e08513555e9a5932c45b5f68be8764ce998df635ae04f6ce7bbcd3b4fd08e2daab7f9059b287ecab4155367b834682633497173f450000"
            ),
            encode_msg(
                "commitment_signed",
                channel_id=b"\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01\x01",
    ...
    def test_encode_decode_msg__init(self):
        ...
        self.assertEqual(
            bfh("00100000000220c2"), encode_msg("init", gflen=0, flen=2, features=bfh("20c2"))
        )
        self.assertEqual(
            bfh(
                "00100000000220c2012043497fd7f826957108f4a30fd9cec3aeba79972084e90ead01ea330900000000"
            ),
            encode_msg(
                "init",
                gflen=0,
        ...
        self.assertEqual(
            ...
            decode_msg(
                bfh(
                    "001000022200000302aaa2012043497fd7f826957108f4a30fd9cec3aeba79972084e90ead01ea330900000000"
                )
            ),
        )
    ...
    def test_decode_onion_error(self):
        ...
        self.assertEqual({"htlc_msat": 399694000, "height": 1938520}, orf.decode_data())
