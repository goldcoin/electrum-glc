    ...
    def resolve_next(self, view: str, wizard_data: dict) -> WizardViewState:
        ...
        if "next" not in nav:
            ...
            if isinstance(view_next, str):
                ...
                raise Exception(
                    f"next handler for view {view} is not callable nor a string literal"
                )
...
class NewWalletWizard(AbstractWizard):
    ...
    def __init__(self, daemon: "Daemon", plugins: "Plugins"):
        ...
        self.navmap = {
            "wallet_name": {"next": "wallet_type"},
            "wallet_type": {"next": self.on_wallet_type},
            "keystore_type": {"next": self.on_keystore_type},
            "create_seed": {"next": "confirm_seed"},
            "confirm_seed": {
            ...
            "have_seed": {
                ...
                and not (self.needs_derivation_path(d) or self.is_multisig(d)),
            },
            ...
            "script_and_derivation": {
                "next": lambda d: (
                    self.wallet_password_view(d)
                    if not self.is_multisig(d)
            ...
            "imported": {"next": "wallet_password", "last": lambda d: self.is_single_password()},
            "wallet_password": {"last": True},
            "wallet_password_hardware": {"last": True},
        }
        self._daemon = daemon
    ...
    def current_cosigner(self, wizard_data: dict) -> dict:
        wdata = wizard_data
        if (
            wizard_data.get("wallet_type") == "multisig"
            and "multisig_current_cosigner" in wizard_data
        ):
    ...
    def on_hardware_device(self, wizard_data: dict, new_wallet=True) -> str:
        ...
        run_hook(
            "init_wallet_wizard", self
        )  # TODO: currently only used for hww, hook name might be confusing
    ...
    def maybe_master_pubkey(self, wizard_data: dict):
        ...
        wizard_data["multisig_master_pubkey"] = self.keystore_from_data(
            wizard_data["wallet_type"], wizard_data
    ...
    def has_duplicate_masterkeys(self, wizard_data: dict) -> bool:
        ...
        xpubs = [
            self.keystore_from_data(wizard_data["wallet_type"], wizard_data).get_master_public_key()
        ]
        for cosigner in wizard_data["multisig_cosigner_data"]:
            data = wizard_data["multisig_cosigner_data"][cosigner]
            xpubs.append(
                self.keystore_from_data(wizard_data["wallet_type"], data).get_master_public_key()
            )
        assert xpubs
    ...
    def is_current_cosigner_hardware(self, wizard_data: dict) -> bool:
        ...
        cosigner_is_hardware = (
            cosigner_data == wizard_data and wizard_data["keystore_type"] == "hardware"
        )
        if (
            "cosigner_keystore_type" in wizard_data
            and wizard_data["cosigner_keystore_type"] == "hardware"
        ):
    ...
    def validate_seed(self, seed: str, seed_variant: str, wallet_type: str):
        ...
        if wallet_type == "2fa" and not is_any_2fa_seed_type(seed_type):
            seed_valid = False
        elif wallet_type == "standard" and seed_type not in [
            "old",
            "standard",
            "segwit",
            "bip39",
            ...
            seed_valid = False
        elif wallet_type == "multisig" and seed_type not in [
            "standard",
            "segwit",
            "bip39",
            "slip39",
        ...
        self._logger.debug(
            f"seed verified: {seed_valid}, type={seed_type}, validation_message={validation_message}"
        )
    ...
    def create_storage(self, path: str, data: dict):
        ...
        if data["wallet_type"] == "standard":
            ...
            db.put("x1", k.dump())
            if (
                "trustedcoin_keepordisable" in data
                and data["trustedcoin_keepordisable"] == "disable"
            ):
            ...
            db.put(
                "wallet_type",
                "%dof%d" % (data["multisig_signatures"], data["multisig_participants"]),
            )
            db.put("x1", k.dump())
            for cosigner in data["multisig_cosigner_data"]:
                cosigner_keystore = self.keystore_from_data(
                    "multisig", data["multisig_cosigner_data"][cosigner]
                )
                if not isinstance(cosigner_keystore, keystore.Xpub):
                    raise Exception(
                        f"unexpected keystore(cosigner) type={type(cosigner_keystore)} in multisig. not bip32."
                    )
        ...
        if k and k.can_have_deterministic_lightning_xprv():
            db.put(
                "lightning_xprv",
                k.get_lightning_xprv(data["password"] if data["encrypt"] else None),
            )
...
class ServerConnectWizard(AbstractWizard):
    ...
    def __init__(self, daemon: "Daemon"):
        ...
        self.navmap = {
            ...
            "proxy_ask": {"next": lambda d: "proxy_config" if d["want_proxy"] else "autoconnect"},
            "proxy_config": {"next": "autoconnect", "accept": self.do_configure_proxy},
            "server_config": {"accept": self.do_configure_server, "last": True},
        }
        self._daemon = daemon
    ...
    def do_configure_proxy(self, wizard_data: dict):
        ...
        self._daemon.network.run_from_another_thread(
            self._daemon.network.set_parameters(net_params)
        )
