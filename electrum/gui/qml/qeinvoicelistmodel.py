        ...
        for i, item in enumerate(self._invoices):
            if item["key"] == key:
                ...
                self.dataChanged.emit(
                    index, index, [self._ROLE_RMAP["status"], self._ROLE_RMAP["status_str"]]
                )
                return
        ...
        for invoice in self._invoices:
            if invoice["status"] != PR_EXPIRED:
                if invoice["expiry"] > 0 and invoice["expiry"] != LN_EXPIRY_NEVER:
                    interval = status_update_timer_interval(
                        invoice["timestamp"] + invoice["expiry"]
                    )
                    if interval > 0:
                        nearest_interval = (
                            nearest_interval if nearest_interval < interval else interval
                        )
