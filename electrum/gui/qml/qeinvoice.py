...
class QEInvoice(QObject, QtEventListener):
    ...
    def update_userinfo(self):
        ...
        if self.invoiceType == QEInvoice.Type.LightningInvoice:
            if self.status in [PR_UNPAID, PR_FAILED]:
                if self.get_max_spendable_lightning() >= amount.satsInt:
                    ...
                    if lnaddr.amount and amount.satsInt < lnaddr.amount * COIN:
                        self.userinfo = _(
                            "Cannot pay less than the amount specified in the invoice"
                        )
            ...
            if self.status in [PR_UNPAID, PR_FAILED]:
                if not (
                    (amount.isMax and self.get_max_spendable_onchain() > 0)
                    or (self.get_max_spendable_onchain() >= amount.satsInt)
                ):
                    self.userinfo = _("Insufficient balance")
            else:
                self.userinfo = {
                    ...
                    PR_BROADCAST: _("Payment in progress...")
                    + " ("
                    + _("broadcast successfully")
                    + ")",
                    PR_UNCONFIRMED: _("Payment in progress...")
                    + " ("
    ...
    def get_max_spendable_lightning(self):
        return (
            self._wallet.wallet.lnworker.num_sats_can_send() if self._wallet.wallet.lnworker else 0
        )
...
class QEInvoiceParser(QEInvoice):
    ...
    def create_onchain_invoice(self, outputs, message, payment_request, uri):
        return self._wallet.wallet.create_invoice(
            outputs=outputs, message=message, pr=payment_request, URI=uri
        )
            ...
            if self._pi.type in [PaymentIdentifierType.SPK, PaymentIdentifierType.OPENALIAS]:
                ...
                if not self._wallet.wallet.has_lightning() and not lninvoice.get_address():
                    self.validationError.emit(
                        "no_lightning",
                        _(
                            "Detected valid Lightning invoice, but Lightning not enabled for wallet and no fallback address found."
                        ),
                    )
                    return
                if (
                    self._wallet.wallet.lnworker
                    and not self._wallet.wallet.lnworker.channels
                    and not lninvoice.get_address()
                ):
                    self.validationWarning.emit(
                        "no_channels",
                ...
                if (
                    self._wallet.wallet.has_lightning()
                    and self._wallet.wallet.lnworker.channels
                    and self._pi.bolt11
                ):
                    lninvoice = self._pi.bolt11
                    self.setValidLightningInvoice(lninvoice)
