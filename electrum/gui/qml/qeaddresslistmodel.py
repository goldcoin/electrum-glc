...
class QEAddressCoinFilterProxyModel(QSortFilterProxyModel):
    ...
    def filterAcceptsRow(self, s_row, s_parent):
        ...
        addridx = parent_model.data(
            parent_model.index(s_row, 0, s_parent), parent_model._ROLE_RMAP["addridx"]
        )
        if addridx is None:  # coin
            ...
            if not self._show_addresses:
                return False
            balance = parent_model.data(
                parent_model.index(s_row, 0, s_parent), parent_model._ROLE_RMAP["balance"]
            )
            numtx = parent_model.data(
                parent_model.index(s_row, 0, s_parent), parent_model._ROLE_RMAP["numtx"]
            )
            if balance.isEmpty and numtx and not self._show_used:
                return False
        if self._filter_text:
            label = parent_model.data(
                parent_model.index(s_row, 0, s_parent), parent_model._ROLE_RMAP["label"]
            )
            address = parent_model.data(
                parent_model.index(s_row, 0, s_parent), parent_model._ROLE_RMAP["address"]
            )
            outpoint = parent_model.data(
                parent_model.index(s_row, 0, s_parent), parent_model._ROLE_RMAP["outpoint"]
            )
            amount_i = parent_model.data(
                parent_model.index(s_row, 0, s_parent), parent_model._ROLE_RMAP["amount"]
            )
            amount = (
                parent_model.wallet.config.format_amount(amount_i.satsInt) if amount_i else None
            )
...
class QEAddressCoinListModel(QAbstractListModel):
    ...
    def initModel(self):
        ...
        c_addresses = (
            self.wallet.get_change_addresses() if self.wallet.wallet_type != "imported" else []
        )
    ...
    def do_update(self, modelindex, modelitem):
        ...
        if modelitem.get("outpoint"):
            ...
            modelitem.update(
                self.addr_to_model(modelitem["type"], modelitem["addridx"], modelitem["address"])
            )
        self._logger.debug(repr(modelitem))
