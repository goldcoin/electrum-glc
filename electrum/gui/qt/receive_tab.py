...
class ReceiveTab(QWidget, MessageBoxMixin, Logger):
    ...
    def __init__(self, window: "ElectrumWindow"):
        ...
        def on_receive_swap():
            if self.receive_swap_button.suggestion:
                ...
                self.window.run_swap_dialog(
                    is_reverse=True, recv_amount_sat=swap_recv_amount_sat, channels=[chan]
                )
        ...
        # menu
        # disabled lightning options in receive tab
        ...
        self.qr_menu_action = menu.addToggle(
            _("Show detached QR code window"), self.window.toggle_qr_window
        )
    ...
    def expiry_dialog(self):
        msg = "".join(
            [
                ...
                " ",
                _(
                    "This information is seen by the recipient if you send them a signed payment request."
                ),
                "\n\n",
                _(
                    "For on-chain requests, the address gets reserved until expiration. After that, it might get reused."
                ),
                " ",
                _(
                    "The goldcoin address never expires and will always be part of this electrum wallet."
                ),
                " ",
                _(
                    "You can reuse a goldcoin address any number of times but it is not good for your privacy."
                ),
                "\n\n",
        ...
        expiry = self.config.WALLET_PAYREQ_EXPIRY_SECONDS
        v = self.window.query_choice(
            msg, pr_expiration_values(), title=_("Expiry"), default_choice=expiry
        )
        if v is None:
            return
    ...
    def update_view_button(self):
        ...
        if i == 0:
            icon, text = read_QIcon("link.png"), _("Goldcoin URI")
        elif i == 1:
            icon, text = read_QIcon("goldcoin.png"), _("Address")
        elif i == 2:
    ...
    def get_tab_data(self):
        i = self.config.GUI_QT_RECEIVE_TABS_INDEX
        if i == 0:
            out = self.URI, self.URI, self.URI_help, _("Goldcoin URI")
        elif i == 1:
        ...
        if addr is None:
            if not self.wallet.is_deterministic():  # imported wallet
                msg = [
                    ...
                    "\n\n",
                    _(
                        "Creating a new payment request will reuse one of your addresses and overwrite an existing request. Continue anyway?"
                    ),
                ]
...
class ReceiveWidget(QWidget):
    ...
    def __init__(
        self, receive_tab: "ReceiveTab", textedit: QWidget, qr: QWidget, help_widget: QWidget
    ):
