...
class SendTab(QWidget, MessageBoxMixin, Logger):
    ...
    def __init__(self, window: "ElectrumWindow"):
        ...
        msg = (
            ...
            + "\n"
            + _("- a Goldcoin address or BIP21 URI")
            + "\n"
            ...
            + _("- an openalias")
            + "\n"
            + _("- an arbitrary on-chain script, e.g.:")
            + " script(OP_RETURN deadbeef)"
            + "\n"
            + "\n"
            + _(
                "You can also pay to many outputs in a single transaction, "
            ...
            + "\n"
            + _(
                "Integers weights can also be used in conjunction with '!', "
        ...
        msg = (
            _("The amount to be received by the recipient.")
            + " "
        ...
        menu.addAction(
            ...
            self.payto_e.on_qr_from_camera_input_btn,
        )
        menu.addAction(
            read_QIcon("picture_in_picture.png"),
            _("Read QR code from screen"),
            self.payto_e.on_qr_from_screenshot_input_btn,
        )
        menu.addAction(
            read_QIcon("file.png"), _("Read invoice from file"), self.payto_e.on_input_file
        )
    ...
    def spend_max(self):
        ...
        assert pi.type in [
            PaymentIdentifierType.SPK,
            PaymentIdentifierType.MULTILINE,
            PaymentIdentifierType.BIP21,
        ...
        if x_fee_amount:
            ...
            msg += "\n" + _("2fa fee: {} (for the next batch of transactions)").format(
                twofactor_fee_str
            )
        frozen_bal = self.get_frozen_balance_str()
        if frozen_bal:
            msg += "\n" + _(
                "Some coins are frozen: {} (can be unfrozen in the Addresses or in the Coins tab)"
            ).format(frozen_bal)
        ...
        *,
        ...
        if is_preview:
            self.window.show_transaction(
                tx, external_keypairs=external_keypairs, payment_identifier=payment_identifier
            )
            return
    ...
    def do_clear(self):
        ...
        self.lock_fields(
            lock_recipient=False, lock_amount=False, lock_max=True, lock_description=False
        )
        self.max_button.setChecked(False)
    ...
    def update_fields(self):
        ...
        if pi.is_multiline():
            self.lock_fields(
                lock_recipient=False, lock_amount=True, lock_max=True, lock_description=False
            )
            self.set_field_validated(
                self.payto_e, validated=pi.is_valid()
            )  # TODO: validated used differently here than openalias
        ...
        lock_recipient = (
            ...
            in [
                ...
                PaymentIdentifierType.OPENALIAS,
                PaymentIdentifierType.BIP70,
                PaymentIdentifierType.BIP21,
                PaymentIdentifierType.BOLT11,
            ]
            and not pi.need_resolve()
        )
        lock_amount = pi.is_amount_locked()
        lock_max = lock_amount or pi.type not in [
            PaymentIdentifierType.SPK,
            PaymentIdentifierType.BIP21,
        ]

        self.lock_fields(
            lock_recipient=lock_recipient,
        ...
        self.save_button.setEnabled(
            not pi_unusable
            and not is_spk_script
            and pi.type not in [PaymentIdentifierType.LNURLP, PaymentIdentifierType.LNADDR]
        )
    ...
    def pay_lightning_invoice(self, invoice: Invoice):
        ...
        if lnworker is None or not lnworker.can_pay_invoice(invoice):
            ...
            can_pay_onchain = invoice.can_be_paid_onchain() and self.wallet.can_pay_onchain(
                invoice.get_outputs(), coins=coins
            )
            ...
            if can_rebalance:
                msg = "".join(
                    [
                        ...
                        "\n",
                        _(
                            "Move funds between your channels in order to increase your sending capacity."
                        ),
                    ]
                )
                choices[0] = msg
            ...
            if r is not None:
                ...
                if r == 0:
                    ...
                    self.window.run_swap_dialog(
                        is_reverse=False, recv_amount_sat=swap_recv_amount_sat, channels=[chan]
                    )
                elif r == 3:
    ...
    def on_notify_merchant_done(self, pi: PaymentIdentifier):
        if pi.is_error():
            ...
            self.logger.debug(
                f"merchant notify result: {pi.merchant_ack_status}: {pi.merchant_ack_message}"
            )
