...
class ChannelsList(MyTreeView):
                ...
                if bal_other != bal_minus_htlcs_other:
                    label += (
                        ...
                        + self.main_window.format_amount(
                            bal_other - bal_minus_htlcs_other, whitespaces=False
                        )
                        + ")"
                    )
            else:
    ...
    def force_close(self, channel_id):
        ...
        msg = (
            "<b>"
            + _("Force-close channel?")
            + "</b><br/>"
            + "<p>"
            + _(
                "If you force-close this channel, the funds you have in it will not be available for {} blocks."
            ).format(to_self_delay)
            + " "
            + _(
                "After that delay, funds will be swept to an address derived from your wallet seed."
            )
            + "</p>"
            + "<u>"
            ...
            + "<p>"
            + _(
                "Funds in this channel will not be recoverable from seed until they are swept back into your wallet, and might be lost if you lose your wallet file."
            )
            + " "
        ...
        if not self.main_window.question(
            msg, title=_("Force-close channel"), rich_text=True, checkbox=backup_cb
        ):
            return
        if self.save_backup:
    ...
    def get_rebalance_pair(self):
        ...
        if len(selected) == 2:
            ...
            if (
                chan1
                and chan2
                and (not self.lnworker.uses_trampoline() or chan1.node_id != chan2.node_id)
            ):
                return chan1, chan2
        return None, None
    ...
    def on_double_click(self, idx):
        ...
        chan = (
            self.lnworker.get_channel_by_id(channel_id) or self.lnworker.channel_backups[channel_id]
        )
    ...
    def create_menu(self, position):
        ...
        if len(selected) == 2:
            ...
            if chan1 and chan2:
                menu.addAction(
                    _("Rebalance channels"), lambda: self.main_window.rebalance_dialog(chan1, chan2)
                )
                menu.exec_(self.viewport().mapToGlobal(position))
            return
        elif len(selected) > 2:
        ...
        cc.addAction(
            _("Node ID"),
            lambda: self.place_text_on_clipboard(chan.node_id.hex(), title=_("Node ID")),
        )
        cc.addAction(
            _("Long Channel ID"),
            lambda: self.place_text_on_clipboard(channel_id.hex(), title=_("Long Channel ID")),
        )
        if not chan.is_backup() and not chan.is_closed():
            ...
            if not chan.is_frozen_for_sending():
                fm.addAction(
                    ...
                    lambda: self.set_frozen(chan, for_sending=True, value=True),
                )
            else:
                fm.addAction(
                    _("Unfreeze for sending"),
                    lambda: self.set_frozen(chan, for_sending=True, value=False),
                )
            if not chan.is_frozen_for_receiving():
                fm.addAction(
                    _("Freeze for receiving"),
                    lambda: self.set_frozen(chan, for_sending=False, value=True),
                )
            else:
                fm.addAction(
                    _("Unfreeze for receiving"),
                    lambda: self.set_frozen(chan, for_sending=False, value=False),
                )
    ...
    def do_update_rows(self, wallet):
        ...
        for chan in channels.values():
            ...
            items[self.Columns.FEATURES].setData(
                ChannelFeatureIcons.from_channel(chan), self.ROLE_CUSTOM_PAINT
            )
    ...
    def update_can_send(self, lnworker: LNWallet):
        msg = (
            _("Can send")
            + " "
            + self.main_window.format_amount(lnworker.num_sats_can_send())
            + " "
            + self.main_window.base_unit()
            + "; "
            + _("can receive")
            + " "
            + self.main_window.format_amount(lnworker.num_sats_can_receive())
            + " "
            + self.main_window.base_unit()
        )
    ...
    def create_toolbar(self, config):
        ...
        menu.addAction(
            read_QIcon("update.png"),
            _("Submarine swap"),
            lambda: self.main_window.run_swap_dialog(),
        )
        menu.addSeparator()
        menu.addAction(
            _("Import channel backup"),
            lambda: self.main_window.do_process_from_text_channel_backup(),
        )
    ...
    def statistics_dialog(self):
        ...
        capacity = (
            self.main_window.format_amount(channel_db.capacity())
            + " "
            + self.main_window.base_unit()
        )
