...
class ScanQRTextEdit(ButtonsTextEdit, MessageBoxMixin):
    ...
    def add_input_buttons(self, config, allow_multi, setText):
        ...
        self.add_qr_input_from_camera_button(
            config=config, show_error=self.show_error, allow_multi=allow_multi, setText=setText
        )
    ...
    def contextMenuEvent(self, e):
        ...
        m.addAction(
            ...
            self.on_qr_from_camera_input_btn,
        )
        m.addAction(
...
class ScanShowQRTextEdit(ButtonsTextEdit, MessageBoxMixin):
    ...
    def __init__(self, text="", allow_multi: bool = False, *, config: SimpleConfig):
        ButtonsTextEdit.__init__(self, text)
        self.setReadOnly(False)
        self.add_qr_input_combined_button(
            config=config, show_error=self.show_error, allow_multi=allow_multi
        )
        self.add_qr_show_button(config=config)
        run_hook("scan_text_edit", self)
        run_hook("show_text_edit", self)
