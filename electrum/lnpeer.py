...
from . import bitcoin, constants, ecc, lnutil, transaction, util
from .bitcoin import DummyAddress, make_op_return
from .crypto import sha256, sha256d
from .ecc import der_sig_from_sig_string, sig_string_from_r_and_s
...
from .lnmsg import FailedToParseMsg, UnknownOptionalMsgType, decode_msg, encode_msg
from .lnonion import (
    InvalidOnionMac,
    InvalidOnionPubkey,
    OnionFailureCode,
    ...
    UnsupportedOnionPacketVersion,
    calc_hops_data_for_payment,
    construct_onion_error,
    new_onion_packet,
    obfuscate_onion_error,
    process_onion_packet,
)
from .lnrouter import RouteEdge, fee_for_edge_msat
...
from .transaction import PartialTxOutput, Sighash, match_script_against_template
from .util import (
    OldTaskGroup,
...
class Peer(Logger):
    ...
    def __init__(
        ...
        *,
        ...
        self.ordered_message_queues = defaultdict(
            asyncio.Queue
        )  # type: Dict[bytes, asyncio.Queue] # for messages that are ordered
    ...
    def send_message(self, message_name: str, **kwargs):
        assert (
            util.get_running_loop() == util.get_asyncio_loop()
        ), "this must be run on the asyncio thread!"
        ...
        self._store_raw_msg_if_local_update(
            raw_msg, message_name=message_name, channel_id=kwargs.get("channel_id")
        )
        self.transport.send_bytes(raw_msg)

    def _store_raw_msg_if_local_update(
        self, raw_msg: bytes, *, message_name: str, channel_id: bytes | None
    ):
    ...
    async def initialize(self):
        ...
        self.send_message(
            ...
            flen=flen,
            features=features,
            init_tlvs={"networks": {"chains": constants.net.rev_genesis_bytes()}},
        )
    ...
    def process_message(self, message: bytes):
        ...
        if message_type in self.ORDERED_MESSAGES:
            ...
            if message_type not in ("error", "warning") and "channel_id" in payload:
                ...
                if chan is None:
                    self.logger.info(
                        f"Received {message_type} for unknown channel {payload['channel_id'].hex()}"
                    )
                    return
                args = (chan, payload)
            else:
    ...
    def on_warning(self, payload):
        ...
        self.logger.info(
            f"remote peer sent warning [DO NOT TRUST THIS MESSAGE]: "
    ...
    def on_error(self, payload):
        ...
        is_known_chan_id = (chan_id in self.channels) or (chan_id in self.temp_id_to_id)
        self.logger.info(
            f"remote peer sent error [DO NOT TRUST THIS MESSAGE]: "
            f"{error_text_bytes_to_safe_str(err_bytes)}. chan_id={chan_id.hex()}. "
            f"{is_known_chan_id=}"
        )
    ...
    async def send_warning(self, channel_id: bytes, message: str | None = None, *, close_connection=False):
        ...
        * SHOULD include the raw, hex-encoded transaction in reply to a funding_created,
        ...
        self.send_message(
            "warning", channel_id=channel_id, data=encoded_data, len=len(encoded_data)
        )
        if close_connection:
            raise GracefulDisconnect

    async def send_error(
        self, channel_id: bytes, message: str | None = None, *, force_close_channel=False
    ):
    ...
    def on_init(self, payload):
        ...
        try:
            ...
            raise GracefulDisconnect(f"remote sent insane features: {e!r}")
        ...
        try:
            ...
            raise GracefulDisconnect(f"{e!s}")
        ...
        if their_networks:
            ...
            if constants.net.rev_genesis_bytes() not in their_chains:
                raise GracefulDisconnect(
                    f"no common chain found with remote. (they sent: {their_chains})"
                )
        # all checks passed
    ...
    def maybe_save_remote_update(self, payload):
        ...
        for chan in self.channels.values():
            if payload["short_channel_id"] in [chan.short_channel_id, chan.get_local_scid_alias()]:
                ...
                self.logger.info(
                    f"saved remote channel_update gossip msg for chan {chan.get_id_for_log()}"
                )
                break
        else:
    ...
    def handle_disconnect(func):
        ...
        async def wrapper_func(self, *args, **kwargs):
            try:
                ...
                self.logger.log(e.log_level, f"Disconnecting: {e!r}")
            except (
                LightningPeerConnectionClosed,
                IncompatibleLightningFeatures,
                aiorpcx.socks.SOCKSError,
            ) as e:
                self.logger.info(f"Disconnecting: {e!r}")
            finally:
        ...
        if self.lnworker == self.lnworker.network.lngossip:
            try:
                ids, complete = await util.wait_for2(
                    self.get_channel_range(), LN_P2P_NETWORK_TIMEOUT
                )
    ...
    def request_gossip(self, timestamp=0):
        if timestamp == 0:
            ...
            self.logger.info(
                f"requesting channel graph since {datetime.fromtimestamp(timestamp).ctime()}"
            )
        self.send_message(
    ...
    async def _message_loop(self):
        ...
        except (TimeoutError, OSError, HandshakeFailed) as e:
            raise GracefulDisconnect(f"initialize failed: {e!r}") from e
    ...
    def upfront_shutdown_script_from_payload(self, payload, msg_identifier: str) -> bytes | None:
    ...
    def make_local_config(
        self, funding_sat: int, push_msat: int, initiator: HTLCOwner, channel_type: ChannelType
    ) -> LocalConfig:
        ...
        local_config.validate_params(
            funding_sat=funding_sat, config=self.network.config, peer_features=self.features
        )
        return local_config
    ...
    def temporarily_reserve_funding_tx_change_address(func):
        ...
        async def wrapper(self: "Peer", *args, **kwargs):
            ...
            wallet = self.lnworker.wallet
            change_addresses = [
                txout.address for txout in funding_tx.outputs() if wallet.is_change(txout.address)
            ]
    ...
    async def channel_establishment_flow(
        ...
        *,
        ...
        per_commitment_secret_first = get_per_commitment_secret_from_seed(
            local_config.per_commitment_secret_seed, RevocationStore.START_INDEX
        )
        ...
        self.logger.debug(
            f"received accept_channel for temp_channel_id={temp_channel_id.hex()}. {payload=}"
        )
        ...
        their_channel_type = (
            accept_channel_tlvs.get("channel_type") if accept_channel_tlvs else None
        )
        if their_channel_type:
            their_channel_type = ChannelType.from_bytes(
                their_channel_type["type"], byteorder="big"
            ).discard_unknown_and_check()
            # if channel_type is set, and channel_type was set in open_channel,
            # and they are not equal types: MUST reject the channel.
            if (
                open_channel_tlvs.get("channel_type") is not None
                and their_channel_type != our_channel_type
            ):
    ...
    def create_channel_storage(
        self, channel_id, outpoint, local_config, remote_config, constraints, channel_type
    ):
        chan_dict = {
    ...
    async def on_open_channel(self, payload):
        ...
        channel_opening_fee = (
            open_channel_tlvs.get("channel_opening_fee") if open_channel_tlvs else None
        )
        ...
        if self.is_channel_type() and channel_type is None:
            raise Exception(
                "sender has advertized option_channel_type, but hasn't sent the channel type"
            )
        ...
        # channel_type was set, and the type is not suitable.
            ...
            channel_type = ChannelType.from_bytes(
                channel_type["type"], byteorder="big"
            ).discard_unknown_and_check()
        ...
        if is_zeroconf and not self.network.config.ZEROCONF_TRUSTED_NODE.startswith(
            self.pubkey.hex()
        ):
    ...
    def schedule_force_closing(self, channel_id: bytes):
        ...
        if not chan:
            self.logger.warning(
                f"tried to force-close channel {channel_id.hex()} but it is not in self.channels yet"
            )
        if ChanCloseOption.LOCAL_FCLOSE in chan.get_close_options():
            self.lnworker.schedule_force_closing(channel_id)
        else:
            self.logger.info(
                f"tried to force-close channel {chan.get_id_for_log()} "
    ...
    def on_channel_reestablish(self, chan, msg):
        ...
        if their_next_local_ctn < 0:
            raise RemoteMisbehaving("channel reestablish: their_next_local_ctn < 0")
        if their_oldest_unrevoked_remote_ctn < 0:
            raise RemoteMisbehaving("channel reestablish: their_oldest_unrevoked_remote_ctn < 0")
        # ctns
        oldest_unrevoked_local_ctn = chan.get_oldest_unrevoked_ctn(LOCAL)
        chan.get_latest_ctn(LOCAL)
        chan.get_next_ctn(LOCAL)
        chan.get_oldest_unrevoked_ctn(REMOTE)
        ...
        def are_datalossprotect_fields_valid() -> bool:
            if their_local_pcp is None or their_claim_of_our_last_per_commitment_secret is None:
                return False
            if their_oldest_unrevoked_remote_ctn > 0:
                our_pcs, __ = chan.get_secret_and_point(
                    LOCAL, their_oldest_unrevoked_remote_ctn - 1
                )
            else:
        ...
        if they_are_ahead:
            ...
            self.logger.warning(
                f"channel_reestablish ({chan.get_id_for_log()}): we are ahead of remote! trying to force-close."
            )
    ...
    async def reestablish_channel(self, chan: Channel):
        ...
        if oldest_unrevoked_remote_ctn == 0:
            ...
            last_rev_secret = chan.revocation_store.retrieve_secret(
                RevocationStore.START_INDEX - last_rev_index
            )
        self.send_message(
            "channel_reestablish",
            channel_id=chan_id,
        ...
        def replay_updates_and_commitsig():
            ...
            self.logger.info(
                f"channel_reestablish ({chan.get_id_for_log()}): replayed {len(replayed_msgs)} unacked messages. "
                f"{[decode_msg(raw_upd_msg)[0] for raw_upd_msg in replayed_msgs]}"
            )

        def resend_revoke_and_ack():
            last_secret, last_point = chan.get_secret_and_point(
                LOCAL, oldest_unrevoked_local_ctn - 1
            )
            next_secret, next_point = chan.get_secret_and_point(
                LOCAL, oldest_unrevoked_local_ctn + 1
            )
            self.send_message(
                "revoke_and_ack",
        ...
        if we_must_resend_revoke_and_ack and not was_revoke_last:
            self.logger.info(
                f"channel_reestablish ({chan.get_id_for_log()}): replaying a revoke_and_ack first."
            )
            resend_revoke_and_ack()
        replay_updates_and_commitsig()
        if we_must_resend_revoke_and_ack and was_revoke_last:
            self.logger.info(
                f"channel_reestablish ({chan.get_id_for_log()}): replaying a revoke_and_ack last."
            )
            resend_revoke_and_ack()
    ...
    def send_channel_ready(self, chan: Channel):
        ...
        second_per_commitment_point = secret_to_pubkey(
            int.from_bytes(
                get_per_commitment_secret_from_seed(
                    chan.config[LOCAL].per_commitment_secret_seed, per_commitment_secret_index
                ),
                "big",
            )
        )
        ...
        if self.features.supports(LnFeatures.OPTION_SCID_ALIAS_OPT):
            ...
            channel_ready_tlvs["short_channel_id"] = {
                "alias": chan.get_local_scid_alias(create_new_if_needed=True)
            }
    ...
    def maybe_send_channel_announcement(self, chan: Channel):
        node_sigs = [
            chan.config[REMOTE].announcement_node_sig,
            chan.config[LOCAL].announcement_node_sig,
        ]
        bitcoin_sigs = [
            chan.config[REMOTE].announcement_bitcoin_sig,
            chan.config[LOCAL].announcement_bitcoin_sig,
        ]
    ...
    def mark_open(self, chan: Channel):
        ...
        if old_state != ChannelState.FUNDED:
            self.logger.info(
                f"cannot mark open ({chan.get_id_for_log()}), current state: {old_state!r}"
            )
            return
    ...
    def maybe_send_announcement_signatures(self, chan: Channel, is_reply=False):
        ...
        if not is_reply and chan.config[REMOTE].announcement_node_sig:
            return
        h = chan.get_channel_announcement_hash()
        bitcoin_signature = ecc.ECPrivkey(chan.config[LOCAL].multisig_key.privkey).sign(
            h, sig_string_from_r_and_s
        )
    ...
    def on_update_fail_htlc(self, chan: Channel, payload):
        ...
        chan.receive_fail_htlc(
            htlc_id, error_bytes=reason
        )  # TODO handle exc and maybe fail channel (e.g. bad htlc_id)
    ...
    def maybe_send_commitment(self, chan: Channel) -> bool:
        ...
        if not chan.has_pending_changes(REMOTE):
            return False
        self.logger.info(
            f"send_commitment. chan {chan.short_channel_id}. ctn: {chan.get_next_ctn(REMOTE)}."
        )
    ...
    def create_onion_for_route(
        ...
        *,
        ...
        if trampoline_onion:
            self.logger.info("adding trampoline onion to final payload")
            ...
            if t_hops_data := trampoline_onion._debug_hops_data:  # None if trampoline-forwarding
                ...
                for i in range(len(t_route)):
                    self.logger.info(
                        f"  {i}: t_node={t_route[i].end_node.hex()} hop_data={t_hops_data[i]!r}"
                    )
        # create onion packet
        payment_path_pubkeys = [x.node_id for x in route]
        onion = new_onion_packet(
            payment_path_pubkeys, session_key, hops_data, associated_data=payment_hash
        )  # must use another sessionkey
        ...
        if cltv_abs > local_height + lnutil.NBLOCK_CLTV_DELTA_TOO_FAR_INTO_FUTURE:
            raise PaymentFailure(
                f"htlc expiry too far into future. (in {cltv_abs-local_height} blocks)"
            )
    ...
    def send_revoke_and_ack(self, chan: Channel):
        if chan.is_closed():
            return
        self.logger.info(
            f"send_revoke_and_ack. chan {chan.short_channel_id}. ctn: {chan.get_oldest_unrevoked_ctn(LOCAL)}"
        )
    ...
    def on_commitment_signed(self, chan: Channel, payload):
        if chan.peer_state == PeerState.BAD:
            return
        self.logger.info(
            f"on_commitment_signed. chan {chan.short_channel_id}. ctn: {chan.get_next_ctn(LOCAL)}."
        )
    ...
    def on_update_fail_malformed_htlc(self, chan: Channel, payload):
        ...
        self.logger.info(
            f"on_update_fail_malformed_htlc. chan {chan.get_id_for_log()}. "
            f"htlc_id {htlc_id}. failure_code={failure_code}"
        )
        if failure_code & OnionFailureCodeMetaFlag.BADONION == 0:
            self.schedule_force_closing(chan.channel_id)
            raise RemoteMisbehaving(
                f"received update_fail_malformed_htlc with unexpected failure code: {failure_code}"
            )
    ...
    def on_update_add_htlc(self, chan: Channel, payload):
        ...
        self.logger.info(f"on_update_add_htlc. chan {chan.short_channel_id}. htlc={htlc!s}")
        if chan.get_state() != ChannelState.OPEN:
            raise RemoteMisbehaving(
                f"received update_add_htlc while chan.get_state() != OPEN. state was {chan.get_state()!r}"
            )
        ...
        *,
        ...
        def log_fail_reason(reason: str):
            self.logger.debug(
                ...
                f"{reason}. inc_htlc={htlc!s}. onion_payload={processed_onion.hop_data.payload}"
            )
        ...
        try:
            _next_chan_scid = processed_onion.hop_data.payload["short_channel_id"][
                "short_channel_id"
            ]  # type: bytes
            next_chan_scid = ShortChannelID(_next_chan_scid)
        except Exception:
            raise OnionRoutingFailure(
                code=OnionFailureCode.INVALID_ONION_PAYLOAD, data=b"\x00\x00\x00"
            )
        try:
            next_amount_msat_htlc = processed_onion.hop_data.payload["amt_to_forward"][
                "amt_to_forward"
            ]
        except Exception:
            raise OnionRoutingFailure(
        ...
        try:
            next_cltv_abs = processed_onion.hop_data.payload["outgoing_cltv_value"][
                "outgoing_cltv_value"
            ]
        except Exception:
            raise OnionRoutingFailure(
        ...
        if not next_chan.can_send_update_add_htlc():
            ...
            raise OnionRoutingFailure(
                code=OnionFailureCode.TEMPORARY_CHANNEL_FAILURE, data=outgoing_chan_upd_message
            )
        if not next_chan.can_pay(next_amount_msat_htlc):
            log_fail_reason(
                "transient error (likely due to insufficient funds): not next_chan.can_pay(amt)"
            )
            raise OnionRoutingFailure(
                code=OnionFailureCode.TEMPORARY_CHANNEL_FAILURE, data=outgoing_chan_upd_message
            )
        ...
        if (
            htlc.cltv_abs - lnutil.MIN_FINAL_CLTV_DELTA_ACCEPTED <= local_height
            or next_cltv_abs <= local_height
        ):
            raise OnionRoutingFailure(
                code=OnionFailureCode.EXPIRY_TOO_SOON, data=outgoing_chan_upd_message
            )
        if (
            max(htlc.cltv_abs, next_cltv_abs)
            > local_height + lnutil.NBLOCK_CLTV_DELTA_TOO_FAR_INTO_FUTURE
        ):
        ...
        if self._maybe_refuse_to_forward_htlc_that_corresponds_to_payreq_we_created(
            htlc.payment_hash
        ):
            log_fail_reason("RHASH corresponds to payreq we created")
            raise OnionRoutingFailure(code=OnionFailureCode.TEMPORARY_NODE_FAILURE, data=b"")
        self.logger.info(
            f"maybe_forward_htlc. will forward HTLC: inc_chan={incoming_chan.short_channel_id}. inc_htlc={htlc!s}. "
    ...
    async def maybe_forward_trampoline(
        ...
        *,
        ...
        forwarding_trampoline_enabled = (
            self.network.config.EXPERIMENTAL_LN_FORWARD_TRAMPOLINE_PAYMENTS
        )
        if not (forwarding_enabled and forwarding_trampoline_enabled):
            self.logger.info("trampoline forwarding is disabled. failing htlc.")
            raise OnionRoutingFailure(code=OnionFailureCode.PERMANENT_CHANNEL_FAILURE, data=b"")
        ...
        if next_peer and next_peer.accepts_zeroconf():
            self.logger.info("JIT: found next_peer")
            for next_chan in next_peer.channels.values():
                if next_chan.can_pay(amt_to_forward):
                    # todo: detect if we can do mpp
                    self.logger.info("jit: next_chan can pay")
                    break
            else:
    ...
    def _maybe_refuse_to_forward_htlc_that_corresponds_to_payreq_we_created(
        self, payment_hash: bytes
    ) -> bool:
    ...
    def maybe_fulfill_htlc(
        ...
        *,
    ...
    ) -> tuple[str | None, bytes | None, Callable[[], Awaitable[str | None]] | None]:
        """
        ...
        if not processed_onion.are_we_final:
            ...
            def callback():
                return self.maybe_forward_htlc(
                            incoming_chan=chan, htlc=htlc, processed_onion=processed_onion
                        )
        ...
        def log_fail_reason(reason: str):
            self.logger.info(
                ...
                f"{reason}. htlc={htlc!s}. onion_payload={processed_onion.hop_data.payload}"
            )

        try:
            amt_to_forward = processed_onion.hop_data.payload["amt_to_forward"]["amt_to_forward"]
        except Exception:
            log_fail_reason("'amt_to_forward' missing from onion")
            raise OnionRoutingFailure(
                code=OnionFailureCode.INVALID_ONION_PAYLOAD, data=b"\x00\x00\x00"
            )
        ...
        chain = self.network.blockchain()
        if chain.is_tip_stale():
            log_fail_reason("our chain tip is stale")
            raise OnionRoutingFailure(code=OnionFailureCode.TEMPORARY_NODE_FAILURE, data=b"")
        local_height = chain.height()
        exc_incorrect_or_unknown_pd = OnionRoutingFailure(
            code=OnionFailureCode.INCORRECT_OR_UNKNOWN_PAYMENT_DETAILS,
            data=amt_to_forward.to_bytes(8, byteorder="big")
            + local_height.to_bytes(4, byteorder="big"),
        )
        try:
            cltv_abs_from_onion = processed_onion.hop_data.payload["outgoing_cltv_value"][
                "outgoing_cltv_value"
            ]
        except Exception:
            log_fail_reason("'outgoing_cltv_value' missing from onion")
            raise OnionRoutingFailure(
                code=OnionFailureCode.INVALID_ONION_PAYLOAD, data=b"\x00\x00\x00"
            )

        if cltv_abs_from_onion > htlc.cltv_abs:
            log_fail_reason("cltv_abs_from_onion != htlc.cltv_abs")
            raise OnionRoutingFailure(
        ...
        try:
            ...
            log_fail_reason("'total_msat' missing from onion")
        ...
        if amt_to_forward > htlc.amount_msat:
            log_fail_reason("amt_to_forward != htlc.amount_msat")
            raise OnionRoutingFailure(
        ...
        try:
            payment_secret_from_onion = processed_onion.hop_data.payload["payment_data"][
                "payment_secret"
            ]  # type: bytes
        except Exception:
            log_fail_reason("'payment_secret' missing from onion")
            raise exc_incorrect_or_unknown_pd
        ...
        if mpp_resolution == RecvMPPResolution.WAITING:
            ...
            log_fail_reason("MPP_TIMEOUT")
            raise OnionRoutingFailure(code=OnionFailureCode.MPP_TIMEOUT, data=b"")
        elif mpp_resolution == RecvMPPResolution.FAILED:
            log_fail_reason("mpp_resolution is FAILED")
            raise exc_incorrect_or_unknown_pd
        ...
        if local_height + MIN_FINAL_CLTV_DELTA_ACCEPTED > htlc.cltv_abs:
            if not already_forwarded:
                log_fail_reason("htlc.cltv_abs is unreasonably close")
                raise exc_incorrect_or_unknown_pd
            else:
                return payment_key, None, None
        ...
        if processed_onion.trampoline_onion_packet:
            ...
            if trampoline_onion.are_we_final:
                ...
                def callback():
                    return self.maybe_forward_trampoline(
                                    payment_hash=payment_hash,
        ...
        if preimage:
            expected_payment_secrets.append(
                derive_payment_secret_from_payment_preimage(preimage)
            )  # legacy secret for old invoices
        if payment_secret_from_onion not in expected_payment_secrets:
            log_fail_reason(
                f"incorrect payment secret {payment_secret_from_onion.hex()} != {expected_payment_secrets[0].hex()}"
            )
            raise exc_incorrect_or_unknown_pd
        ...
        if not (invoice_msat is None or invoice_msat <= total_msat <= 2 * invoice_msat):
            log_fail_reason(
                f"total_msat={total_msat} too different from invoice_msat={invoice_msat}"
            )
            raise exc_incorrect_or_unknown_pd
        ...
        if hold_invoice_callback and not preimage:
            def callback():
                return hold_invoice_callback(payment_hash)
            return payment_key, None, callback

        if not preimage:
            if not already_forwarded:
                log_fail_reason(
                    f"missing preimage and no hold invoice callback {payment_hash.hex()}"
                )
                raise exc_incorrect_or_unknown_pd
            else:
                return payment_key, None, None
        ...
        self.logger.info(
            f"maybe_fulfill_htlc. will FULFILL HTLC: chan {chan.short_channel_id}. htlc={htlc!s}"
        )
    ...
    def fulfill_htlc(self, chan: Channel, htlc_id: int, preimage: bytes):
        ...
        self.send_message(
            "update_fulfill_htlc", channel_id=chan.channel_id, id=htlc_id, payment_preimage=preimage
        )
    ...
    def fail_malformed_htlc(self, *, chan: Channel, htlc_id: int, reason: OnionRoutingFailure):
        ...
        if not (reason.code & OnionFailureCodeMetaFlag.BADONION and len(reason.data) == 32):
            raise Exception(
                f"unexpected reason when sending 'update_fail_malformed_htlc': {reason!r}"
            )
        self.received_htlcs_pending_removal.add((chan, htlc_id))
        chan.fail_htlc(htlc_id)
        self.send_message(
    ...
    def on_revoke_and_ack(self, chan: Channel, payload):
        if chan.peer_state == PeerState.BAD:
            return
        self.logger.info(
            f"on_revoke_and_ack. chan {chan.short_channel_id}. ctn: {chan.get_oldest_unrevoked_ctn(REMOTE)}"
        )
    ...
    def maybe_update_fee(self, chan: Channel):
        ...
        if feerate_per_kw < chan_fee / 2:
            ...
            return
        self.logger.info(
            f"(chan: {chan.get_id_for_log()}) current pending feerate {chan_fee}. "
    ...
    async def close_channel(self, chan_id: bytes):
        ...
        try:
            ...
            if txid is None:
                raise Exception(
                    "The remote peer did not send their final signature. The channel may not have been be closed"
                )
        return txid
    ...
    async def on_shutdown(self, chan: Channel, payload):
        ...
        if self.is_upfront_shutdown_script() and their_upfront_scriptpubkey:
            ...
            if self.is_shutdown_anysegwit() and match_script_against_template(
                their_scriptpubkey, transaction.SCRIPTPUBKEY_TEMPLATE_ANYSEGWIT
            ):
                pass
            elif match_script_against_template(
                their_scriptpubkey, transaction.SCRIPTPUBKEY_TEMPLATE_WITNESS_V0
            ):
                pass
            else:
                await self.send_warning(
    ...
    async def send_shutdown(self, chan: Channel):
        ...
        self.send_message(
            "shutdown", channel_id=chan.channel_id, len=len(scriptpubkey), scriptpubkey=scriptpubkey
        )
    ...
    def get_shutdown_fee_range(self, chan, closing_tx, is_local):
        ...
        if config.TEST_SHUTDOWN_FEE:
            ...
            if our_fee is None:  # fallback
                self.logger.warning(
                    "got no fee estimates for co-op close! falling back to chan.get_latest_fee"
                )
                our_fee = max_fee
    ...
    async def _shutdown(self, chan: Channel, payload, *, is_local: bool):
        ...
        def send_closing_signed(our_fee, our_fee_range, drop_remote):
            ...
            if our_fee_range:
                ...
                closing_signed_tlvs = {}
            our_sig, closing_tx = chan.make_closing_tx(
                our_scriptpubkey, their_scriptpubkey, fee_sat=our_fee, drop_remote=drop_remote
            )
        ...
        async def receive_closing_signed():
            ...
            # perform checks
            our_sig, closing_tx = chan.make_closing_tx(
                our_scriptpubkey, their_scriptpubkey, fee_sat=their_fee, drop_remote=False
            )
            if verify_signature(closing_tx, their_sig):
                drop_remote = False
            else:
                our_sig, closing_tx = chan.make_closing_tx(
                    our_scriptpubkey, their_scriptpubkey, fee_sat=their_fee, drop_remote=True
                )
                if verify_signature(closing_tx, their_sig):
                    drop_remote = True
                else:
            ...
            if not drop_remote and to_remote_set:
                ...
                transaction.check_scriptpubkey_template_and_dust(
                    their_scriptpubkey, to_remote_amount
                )
        ...
        def choose_new_fee(our_fee, our_fee_range, their_fee, their_fee_range, their_previous_fee):
            ...
            if (
                ...
                and not self.network.config.TEST_SHUTDOWN_FEE_RANGE
            ):
                # SHOULD set max_fee_satoshis to at least the max_fee_satoshis received
                our_fee_range["max_fee_satoshis"] = max(
                    their_fee_range["max_fee_satoshis"], our_fee_range["max_fee_satoshis"]
                )
                # SHOULD set min_fee_satoshis to a fairly low value
                our_fee_range["min_fee_satoshis"] = min(
                    their_fee_range["min_fee_satoshis"], our_fee_range["min_fee_satoshis"]
                )
            ...
            if fee_range_sent and (
                our_fee_range["min_fee_satoshis"] <= their_fee <= our_fee_range["max_fee_satoshis"]
            ):
            ...
            # the receiving node, if the message contains a fee_range
                ...
                overlap_min = max(
                    our_fee_range["min_fee_satoshis"], their_fee_range["min_fee_satoshis"]
                )
                overlap_max = min(
                    our_fee_range["max_fee_satoshis"], their_fee_range["max_fee_satoshis"]
                )
                ...
                if their_previous_fee and not (
                    min(our_fee, their_previous_fee) < their_fee < max(our_fee, their_previous_fee)
                ):
                    # SHOULD fail the connection.
                    raise Exception(
                        "Their fee is not between our last sent and their last sent fee."
                    )
        ...
        while True:
            ...
            if our_fee == their_fee:
                break
            our_fee, our_fee_range = choose_new_fee(
                our_fee, our_fee_range, their_fee, their_fee_range, their_previous_fee
            )
    ...
    async def htlc_switch(self):
        ...
        while True:
            ...
            for _chan_id, chan in self.channels.items():
                if not chan.can_send_ctx_updates():
                    continue
                self.maybe_send_commitment(chan)
                done = set()
                ...
                for htlc_id, (
                    ...
                    onion_packet_hex,
                    forwarding_key,
                ) in unfulfilled.items():
                    if not chan.hm.is_htlc_irrevocably_added_yet(
                        htlc_proposer=REMOTE, htlc_id=htlc_id
                    ):
                        continue
                    ...
                    try:
                        ...
                        try:
                            ...
                            error_bytes = construct_onion_error(
                                e, onion_packet.public_key, our_onion_private_key=self.privkey
                            )
                        if error_bytes:
                            error_bytes = obfuscate_onion_error(
                                error_bytes,
                    ...
                    if preimage or error_reason or error_bytes:
                        if preimage:
                            ...
                            self.fail_malformed_htlc(
                                chan=chan, htlc_id=htlc.htlc_id, reason=error_reason
                            )
                        done.add(htlc_id)
                # cleanup
                for htlc_id in done:
    ...
    def process_unfulfilled_htlc(
        ...
        *,
    ...
    ) -> tuple[bytes | None, Union[bool, None, tuple[str, int]], bytes | None]:
        """
        ...
        processed_onion = self.process_onion_packet(
            onion_packet, payment_hash=payment_hash, onion_packet_bytes=onion_packet_bytes
        )
        ...
        if error_bytes or error_reason or preimage:
            self.lnworker.maybe_cleanup_forwarding(
                payment_key, chan.get_scid_or_local_alias(), htlc
            )
    ...
    def process_onion_packet(
        ...
        *,
        ...
        try:
            ...
            raise OnionRoutingFailure(
                code=OnionFailureCode.INVALID_ONION_VERSION, data=failure_data
            )
