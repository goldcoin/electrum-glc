...
class SimpleConfig(Logger):
    ...
    def convert_version_3(self):
        ...
        if isinstance(base_unit, str):
            ...
            map_ = {"glc": 8, "mglc": 5, "uglc": 2, "bits": 2, "sat": 0}
    ...
    def setfeerate(self, fee_method: str, value: int):
        if fee_method == "mempool":
            ...
            raise Exception(
                f"Invalid parameter: {fee_method}. Valid methods are: ETA, mempool, static."
            )
    ...
    def estimate_fee(
        self, size: Union[int, float, Decimal], *, allow_fallback_to_static_rates: bool = False
    ) -> int:
        fee_per_kb = self.fee_per_kb()
        if fee_per_kb is None:
    ...
    NETWORK_MAX_INCOMING_MSG_SIZE = ConfigVar(
        "network_max_incoming_msg_size", default=1_000_000, type_=int
    )  # in bytes
    ...
    WALLET_MERGE_DUPLICATE_OUTPUTS = ConfigVar(
        "wallet_merge_duplicate_outputs",
        default=False,
        type_=bool,
        short_desc=lambda: _("Merge duplicate outputs"),
        long_desc=lambda: _(
            "Merge transaction outputs that pay to the same address into "
    ...
    WALLET_COIN_CHOOSER_OUTPUT_ROUNDING = ConfigVar(
        ...
        long_desc=lambda: (
            _(
                "Set the value of the change output so that it has similar precision to the other outputs."
            )
            + "\n"
    ...
    WALLET_UNCONF_UTXO_FREEZE_THRESHOLD_SAT = ConfigVar(
        "unconf_utxo_freeze_threshold", default=5_000, type_=int
    )
    ...
    WALLET_PAYREQ_EXPIRY_SECONDS = ConfigVar(
        "request_expiry", default=invoices.PR_DEFAULT_EXPIRATION_WHEN_CREATING, type_=int
    )
    ...
    WALLET_SEND_CHANGE_TO_LIGHTNING = ConfigVar(
        ...
        long_desc=lambda: _(
            "If possible, send the change of this transaction to your channels, with a submarine swap"
        ),
    )
    ...
    FX_EXCHANGE = ConfigVar(
        "use_exchange", default="CoinGecko", type_=str
    )  # default exchange should ideally provide historical rates
...
Note you are at risk of losing the funds in the swap, if the funding transaction never confirms."""
    ...
    LIGHTNING_MAX_FUNDING_SAT = ConfigVar(
        "lightning_max_funding_sat", default=LN_MAX_FUNDING_SAT_LEGACY, type_=int
    )
    LIGHTNING_LEGACY_ADD_TRAMPOLINE = ConfigVar(
        "lightning_legacy_add_trampoline",
        default=False,
        type_=bool,
...
This will result in longer routes; it might increase your fees and decrease the success rate of your payments."""
    ...
    EXPERIMENTAL_LN_FORWARD_PAYMENTS = ConfigVar(
        "lightning_forward_payments", default=False, type_=bool
    )
    EXPERIMENTAL_LN_FORWARD_TRAMPOLINE_PAYMENTS = ConfigVar(
        "lightning_forward_trampoline_payments", default=False, type_=bool
    )
    TEST_FAIL_HTLCS_WITH_TEMP_NODE_FAILURE = ConfigVar(
        "test_fail_htlcs_with_temp_node_failure", default=False, type_=bool
    )
    ...
    GUI_QT_ADDRESSES_TAB_SHOW_TOOLBAR = ConfigVar(
        "show_toolbar_addresses", default=False, type_=bool
    )
    GUI_QT_TX_DIALOG_FETCH_TXIN_DATA = ConfigVar(
        "tx_dialog_fetch_txin_data",
        default=False,
        type_=bool,
    ...
    GUI_QML_PREFERRED_REQUEST_TYPE = ConfigVar(
        "preferred_request_type", default="bolt11", type_=str
    )
    GUI_QML_USER_KNOWS_PRESS_AND_HOLD = ConfigVar(
        "user_knows_press_and_hold", default=False, type_=bool
    )
    ...
    WRITE_LOGS_TO_DISK = ConfigVar(
        ...
        long_desc=lambda: _(
            "Debug logs can be persisted to disk. These are useful for troubleshooting."
        ),
    )
    ...
    WATCHTOWER_CLIENT_ENABLED = ConfigVar(
        ...
        long_desc=lambda: " ".join(
            [
                ...
                _(
                    "Check our online documentation if you want to configure Electrum as a watchtower."
                ),
            ]
        ),
    )
    ...
    PAYSERVER_ALLOW_CREATE_INVOICE = ConfigVar(
        "payserver_allow_create_invoice", default=False, type_=bool
    )
