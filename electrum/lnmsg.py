...
class UnknownOptionalMsgType(UnknownMsgType):
    pass
...
class MalformedMsg(FailedToParseMsg):
    pass
...
class MsgTrailingGarbage(MalformedMsg):
    pass


class MsgInvalidFieldOrder(MalformedMsg):
    pass


class UnexpectedFieldSizeForEncoder(MalformedMsg):
    pass
...
def _write_field(
    *, fd: io.BytesIO, field_type: str, count: Union[int, str], value: Union[bytes, int]
) -> None:
    if not fd:
        raise Exception()
    if isinstance(count, int):
...
def _resolve_field_count(
    field_count_str: str, *, vars_dict: dict, allow_any=False
) -> Union[int, str]:
...
class LNSerializer:
    ...
    def __init__(self, *, for_onion_wire: bool = False):
        ...
        self.in_tlv_stream_get_tlv_record_scheme_from_type = (
            {}
        )  # type: Dict[str, Dict[int, List[Sequence[str]]]]
        ...
        with open(path, newline="") as f:
            ...
            for row in csvreader:
                ...
                if row[0] == "msgtype":
                    ...
                    assert (
                        msg_type_bytes not in self.msg_scheme_from_type
                    ), f"type collision? for {msg_type_name}"
                    assert (
                        msg_type_name not in self.msg_type_from_name
                    ), f"type collision? for {msg_type_name}"
                    row[2] = msg_type_int
                    ...
                    if tlv_stream_name not in self.in_tlv_stream_get_tlv_record_scheme_from_type:
                        self.in_tlv_stream_get_tlv_record_scheme_from_type[tlv_stream_name] = (
                            OrderedDict()
                        )
                    ...
                    assert (
                        tlv_record_type
                        not in self.in_tlv_stream_get_tlv_record_scheme_from_type[tlv_stream_name]
                    ), f"type collision? for {tlv_stream_name}/{tlv_record_name}"
                    assert (
                        tlv_record_name
                        not in self.in_tlv_stream_get_record_type_from_name[tlv_stream_name]
                    ), f"type collision? for {tlv_stream_name}/{tlv_record_name}"
                    assert (
                        tlv_record_type
                        not in self.in_tlv_stream_get_record_type_from_name[tlv_stream_name]
                    ...
                    if (
                        ...
                        raise Exception(
                            f"tlv record types must be listed in monotonically increasing order for stream. "
    ...
    def write_tlv_stream(self, *, fd: io.BytesIO, tlv_stream_name: str, **kwargs) -> None:
        ...
        for (
            tlv_record_type,
            scheme,
        ) in scheme_map.items():  # note: tlv_record_type is monotonically increasing
            tlv_record_name = self.in_tlv_stream_get_record_name_from_type[tlv_stream_name][
                tlv_record_type
            ]
                ...
                for row in scheme:
                    if row[0] == "tlvtype":
                        ...
                        field_count = _resolve_field_count(
                            field_count_str, vars_dict=kwargs[tlv_record_name], allow_any=True
                        )
    ...
    def read_tlv_stream(self, *, fd: io.BytesIO, tlv_stream_name: str) -> dict[str, dict[str, Any]]:
        ...
        while _num_remaining_bytes_to_read(fd) > 0:
            ...
            if not (tlv_record_type > last_seen_tlv_record_type):
                raise MsgInvalidFieldOrder(
                    f"TLV records must be monotonically increasing by type. "
            ...
            try:
                ...
                if tlv_record_type % 2 == 0:
                    ...
                    raise UnknownMandatoryTLVRecordType(
                        f"{tlv_stream_name}/{tlv_record_type}"
                    ) from None
                else:
            ...
            with io.BytesIO(tlv_record_val) as tlv_record_fd:
                for row in scheme:
                    ...
                    if row[0] == "tlvtype":
                        ...
                        field_count_str = row[5]
                        field_count = _resolve_field_count(
                            field_count_str, vars_dict=parsed[tlv_record_name], allow_any=True
                        )
                        # print(f">> count={field_count}. parsed={parsed}")
                        parsed[tlv_record_name][field_name] = _read_field(
                            fd=tlv_record_fd, field_type=field_type, count=field_count
                        )
                    else:
                        raise Exception(f"unexpected row in scheme: {row!r}")
                if _num_remaining_bytes_to_read(tlv_record_fd) > 0:
                    raise MsgTrailingGarbage(
                        f"TLV record ({tlv_stream_name}/{tlv_record_name}) has extra trailing garbage"
                    )
        return parsed
                ...
                if row[0] == "msgtype":
                    ...
                    if field_name == "tlvs":
                        ...
                        if tlv_stream_name in kwargs:
                            self.write_tlv_stream(
                                fd=fd, tlv_stream_name=tlv_stream_name, **(kwargs[tlv_stream_name])
                            )
                        continue
                    try:
                ...
                for row in scheme:
                    ...
                    if row[0] == "msgtype":
                        ...
                        parsed[field_name] = _read_field(
                            fd=fd, field_type=field_type, count=field_count
                        )
                    else:
                        raise Exception(f"unexpected row in scheme: {row!r}")
